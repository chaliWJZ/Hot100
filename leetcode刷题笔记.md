...
class Solution {
	...
	// ‘特殊点’--->"排列" 的题目的话，就是要传入一个 used 数组 !! 而不是 startIndex了
     // used数组[]，主要是 标记单个答案temp里面，我们选了哪些元素。这样就可以使用， 前面没用过的元素，然后使用过的元素我们也可以不再用它  
   ...
   //used数组[] 就是记录， 此时的temp 里已经选的元素，如果这个元素已经在了，那么就跳过 continue 
            if (used[i]==true)		
                continue;
...
```java
// 这道题目，相对于 46题的 原始 “全排列”题目的话，就是 数组中存在 重复的元素，所以我们要去重 ！！                                                               其实就是 多加了两行代码
class Solution {
    ...
    // 我们去重主要是，同⼀"树层"的去重 。 所以只需判断 used[i - 1]==false，使用过则直接跳过
...
class Solution {
   ...
   // dp[i] 是指，考虑下标i（包括i）以内的房屋，最多可以偷窃的金额为 dp[i] 
        ...
        // 遍历顺序 。依赖前面的状态，所以是从前往后。
		for (int i = 2; i < nums.length; i++) {
            
           /** 
     如果偷第i房间，那么dp[i] = dp[i - 2] + nums[i]，也就是说 第i-1房一定是不考虑偷的，找出 下标i-2（包括i-2）以内的房屋，最多可以偷窃的金额为dp[i-2] 加上第i房间偷到的钱。

	如果不偷第i房间，那么dp[i] = dp[i - 1]，也就是考虑i-1房，但并不是一定要偷i-1房
...
class Solution {
    ...
    public int rob(int[] nums) {
       ...
       这道题 II ，房间环形排列 意味着 第一间 和 最后一间 不能同时选择，所以我们可以分成两种情况来讨论：
		...
		两种情况中取最大值，这样我们就把 环形问题 转化为了 两个单排列的子问题。    
       **/
   ...
   // copyOfRange() 方法，方便传入某个数组下标范围的 新数组，可以直接调用下面的 打家劫舍I的代码
        	// 这个方法是不包括 有边界的。。。
...
class Solution {
  ...
  // 如果切割线 startIndex 的起始位置已经大于等于 s 的长度，说明已经找到 一组分割方案了，递归出口
        ...
        for (int i = startIndex; i < s.length(); i++) {
            ...
            // 通过[startIndex,i]区间，就能获取在 s 中的子串。然后判断如果是回文串的话
            			// 那么肯定要调用下面自定义的 判断回文串函数 isPalindrome()
...
```java
// 这道题，和前面的  "连续"子串的回文问题 不太一样。这道题 也可以 “不连续”的。。所以不能用中心扩散来做了，只能用 ---> 动态规划 来做
...
public class Solution {
    ...
    // 先根据 dp递推公式，画出的2*2的方格，可以判断是 从下往上 遍历↑，然后再从 左往右遍历→
        		/**      _________	________
        			   |dp[i,j-1] | dp[i,j] |
        			   |dp[i+1,j-1]| dp[i+1,j] |
    ...
    // 当 i下标对应的字符和 j下标对应的字符  不相等 情况下。那么就只可能是 下标范围 [i,j-1]或者下标范围 [i + 1][j] 的dp数组中的 最大值 
        ...
        // 返回的就是 下标范围 [0,n-1] 的 dp数组。
 // 比如是s ="abbc" ，n 是字符串长度4 。 那么 [0,3] 是指整个原始字符串s "abbc"， 			       下标   0123						那么它的 最长回文子序列的长度是 dp[0][n - 1]
...
public class Solution {
 ...
 // 所以此时的 dp[i][j]是指，以下标i-1为结尾的字符串word1，和以下标j-1为结尾的字符串word2，最近编辑距离为dp[i][j]
        int[][] dp = new int[len1 + 1][len2 + 1];
        
        // 初始化dp数组，第1列，当 word2 长度为 0 时。将 word1 的全部删除即可，编辑距离就是非空字符串word1的长度 i
        for (int i = 1; i <= len1; i++) {
            dp[i][0] = i;
        }
        
        // 初始化dp数组，第1行，当 word1 长度为 0 时。插入所有 word2 的字符即可，编辑距离就是非空字符串word2的长度 j
        for (int j = 1; j <= len2; j++) {
            dp[0][j] = j;
        }

       
 //dp数组遍历顺序，因为dp[i][j] 依赖左上角，上边元素，左边元素 ，所以是 从上到下↓，从左到右→遍历
        for (int i = 1; i <= len1; i++) {
            for (int j = 1; j <= len2; j++) {
                
                // 这是最佳情况，word1和word2的此时 两个字符 相等的时候
                if (word1.charAt(i - 1) == word2.charAt(j - 1)) {
                  //此时的dp[i][j]递推公式等于 它们分别去掉一个字符的 编辑距离dp[i-1][j-1]
                    dp[i][j] = dp[i - 1][j - 1];
                    
                }
                else{
                    
         // 两个字符 不相等，那么就是对 word1 增删改 操作，以下三种情况中选出步骤最少的。
                    // 1、选左边元素，就是在word1后面 增加一个 和 word2一样的末尾字符
                    int insert = dp[i][j - 1];
                    // 2、选左上角元素,就是把 word1的末尾字符 替换成 和 word2一样的末尾字符
                int replace = dp[i - 1][j - 1];
                // 3、选上边元素dp[],就是把 word1的末尾字符 删掉
                int delete = dp[i - 1][j];
                    
              //此时的递推公式，依赖于 就是取这三者的最小值，然后再 +1 就可以了
...
​	   然后是递归调用自己的 **<u>回溯函数</u>**，一般是，如果自己元素取过了，不能重复取，要**递归取自己元素的下一个**，那么就传入**backtracking**(自带参数，额外参数，**i+1**) 或  **backtracking**(自带参数，额外参数，**startIndex+1**)！！！如果是 可以自己元素取过了，下一层递归 **还可以从自己元素 **开始取，那么就是传入 **backtracking**(自带参数，额外参数，**i**) 或  **backtracking**(自带参数，额外参数，**startIndex**)
...
​		这里唯一的区别就是，需要获得**每次切割**的 **子串**，基本上就是  **[startIndex,i]**  左闭右闭区间，所以  经常需要配合 字符串的 **substring**(s,startIndex,**i+1**)方法，记得是 **i+1**，因为右边下标是 取不到的。然后的还要经常**配合** 下面定义的**次函数** 是否符合条件，所以写法变成了  for(....) {  **if ( 次函数 ) { 回溯逻辑... }** }，只有符合条件的子串， 才需要回溯递归。

​		**有时候，有可能**, **只需要**对**原始的 字符串**String进行 **操作 **就可以了，所以的话就是 **不需要temp.add()**了

### 子集

​	也是建立在组合的基础上。。	但是  **<u>求子集</u>** 问题  和  求组合问题 **不一样**。**求组合**问题是**收集叶子节点**，**<u>求子集</u>**问题是收集**<u>所有节点</u>**！！
...
​	   主要区别在于，参数的话，主要就是不需要startIndex参数了 × 然后的话只要一个 **used数组[ ]**  √  ，

传入的初始值 就是   **boolean[] used = new boolean[nums.length]** 。
...
​		  这里主要区别，就是 for循环的 （） 括号里面，**i = 0** 了！！！要从0下标开始，因为"排列"问题，12和21 两个答案，都是要求出来的。所以 我们都是可以选择 **前面的元素**，那么就是要从 i =0 下标开始选。

​			然后for循环的 {...} 递归逻辑里面的话，**每次都要** 判断  **if(used[i]==true)** ，就要 **coninue 跳过**。毕竟上面选过的元素   只能选一次，不能重复选择。。
